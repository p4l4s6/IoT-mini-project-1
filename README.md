...
To run this project you can use already setup cloud server mentioned in below or you can setup your own cloud server by using the instruction in [Preparing the Backend section](#preparing-the-backend). If you use the below cloud server skip the [Preparing the Backend section](#preparing-the-backend).
Cloud server:
```
IPv4: 16.170.238.248
...
nano config.conf
```
put the below configuration in the `config.conf` file:
```
...
This code should be running on your own laptop/desktop. You need `python 3.9+` installed in your laptop or desktop to run the below instructions.

Step 1: Clone the repository
```
git clone https://github.com/p4l4s6/IoT-mini-project-1.git
```
Step 2: Go to the code directory using below command
```
cd IoT-mini-project-1
```
...
If you are using your own server, open the `frontend.py` with any code editor or notepad and replace
```
mqtt_broker_address = "16.170.238.248"
mqtt_port = 1886
```
in lines 6 and 7 with your own cloud server IP address and port.

Step 5: Run the frontend code
```
python frontend.py
```
...
These instructions are for running your `FIT IoT-Lab` nodes. The below cloud server is already preconfigured, you can use your own cloud server if you have setup the cloud server via [Preparing the Backend section](#preparing-the-backend) section. Make sure your own cloud server has IPv6 and has the open ports `1885` and `1886`.
...
replace `ID_OF_BORDER_NODE` with 1st node id you got from step3. if you are using grenoble site you can use this `2001:660:5307:3101::/64` and
if you are using other site make sure you are using the IP range that is supported in that site. you can get the supported IP range from this [URL](https://www.iot-lab.info/legacy/tutorials/understand-ipv6-subnetting-on-the-fit-iot-lab-testbed/index.html)
...
Step 12:  Open the shell of the second node
```
...
```
if you are using your own cloud server replace `2a05:d016:eea:c800:ca30:d1a0:b5e6:7e41` this IP with your cloud IPv6 address and `1885` with your cloud RSMB port.
